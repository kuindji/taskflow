...
import { removeSessionFromOwner } from "./services/session-owner";
```
...
First, extract the existing nested `removeSessionFromOwner(...)` helper from `packages/backend/src/handlers/session.ts` into a reusable `packages/backend/src/services/session-owner.ts` helper so manual sessions and flow-spawned sessions share the exact same owner cleanup path.

Then create `FlowRunner` after `ptyManager` and `taskStore` are created:
...
const flowRunner = new FlowRunner({
  ...
  spawnSession: async (opts) => {
    ...
    if (opts.sessionType === "shell") {
      // Shell steps execute the prompt through a real shell, not as a raw executable path.
      command = "/bin/sh";
...
Update `registerSessionHandlers` to import and reuse the shared `removeSessionFromOwner(...)` helper instead of keeping a private copy. This avoids session-tab drift between manual and flow-driven sessions.
...
```bash
git add packages/backend/src/index.ts packages/backend/src/handlers/session.ts packages/backend/src/services/session-owner.ts

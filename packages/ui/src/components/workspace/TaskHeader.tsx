...
export function TaskHeader({ task, project }: TaskHeaderProps) {
  ...
  return (
    <div className="px-3 py-1.5 border-b border-border flex items-center gap-2">
      ...
      >
        {fileExplorerOpen ? <PanelLeftClose className="h-3.5 w-3.5" /> : <PanelLeftOpen className="h-3.5 w-3.5" />}
      </Button>
      ...
      >
        {taskInfoOpen ? <PanelRightClose className="h-3.5 w-3.5" /> : <PanelRightOpen className="h-3.5 w-3.5" />}
      </Button>
    </div>
  );
}

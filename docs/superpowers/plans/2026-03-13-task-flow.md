...
`removeSessionFromOwner` already exists as a top-level exported function in `packages/backend/src/handlers/session.ts`. Import it directly — no new file needed.
...
const useFlowStore = create<FlowStore>((set) => ({
  ...
  async fetchFlowRuns(taskId) {
    ...
    set((s) => {
      if (run) {
        return { activeRuns: { ...s.activeRuns, [taskId]: run } };
      }
  ...
  applyRunUpdate(run) {
    set((s) => ({
      activeRuns: { ...s.activeRuns, [run.taskId]: run },
    }));
  },
}));

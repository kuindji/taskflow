...
- For each target, runs 3 lightweight git commands:
  ...
  3. `git --no-optional-locks status --porcelain=v1 -z -unormal` — file list (uses `-unormal` instead of `-uall` to avoid recursive enumeration)
...
- `invalidate(id)` method forces immediate re-poll for a target on the next cycle (called after mutations like stage, unstage, commit, revert)
...
Broadcast payload: `{ targetId: string; stats: ChangeStats }` — sent per-target only when values change.

### UI: Simplified `diff-store.ts`

- Drops all polling logic: `startPolling`, `fetchDiff`, `fetchAllDiffs`, file watcher subscription, visibility listener
...
- `packages/ui/src/components/sidebar/TaskSidebar.tsx` — remove diff polling setup
...
- `file-watcher.ts` — unrelated to diff stats

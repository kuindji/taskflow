...
import { isSafeLabel } from "../discovery";
...
/**
 * One manager per process that owns ssh children. State is per manager, so two
 * managers side by side never see or stop each other's tunnels.
 */
function createTunnelManager() {
    ...
    function openTunnel(record: BackendRecord, backendPort: number): Promise<TunnelResult> {
        ...
        if (existing?.established)
            return Promise.resolve({ ok: true, localPort: existing.localPort });
...
type TunnelManager = ReturnType<typeof createTunnelManager>;

export { createTunnelManager };
export type { TunnelManager, TunnelResult };

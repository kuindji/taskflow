...
**Architecture:** Backend-driven orchestrator — `FlowRunner` service owns execution logic, spawns sessions via existing `PtyManager`, advances steps when agents signal completion via `taskflow-cli`. UI is a pure view/control layer with a Zustand store and three new components (FlowPanel, FlowManagementDialog, TaskHeader dropdown).
...
```bash
git add packages/shared/src/types/flow.ts packages/shared/src/types/index.ts
...
describe("step definitions", () => {
  ...
  test("deleteStep removes step", async () => {
    const step = {
      id: "step-1",
      name: "Planning",
      prompt: "Write a plan",
...
class FlowRunner {
  ...
  async jumpToStep(taskId: string, flowId: string, targetIndex: number): Promise<void> {
    ...
    // Clean up current session mapping
    const currentStep = run.steps[run.currentStepIndex];
    if (currentStep?.sessionId) {
      this.sessionFlowMap.delete(currentStep.sessionId);
    }
    ...
    if (targetIndex > run.currentStepIndex) {
      // Forward jump — skip intermediate
  ...
  private async launchStep(
    ...
    const prompt = this.buildStepPrompt(resolved.prompt, taskDescription);
    ...
    const sessionId = await this.deps.spawnSession({
      ...
      agentOptions: resolved.agentOptions as Record<string, unknown> | undefined,
      flowId: flow.id,
  ...
  private buildStepPrompt(stepPrompt: string, taskDescription: string): string {
    return [
      `## Task Description\n\n${taskDescription}`,
      `## Step Instructions\n\n${stepPrompt}`,
      `## Taskflow CLI`,
...
artifact)
  case "$2" in
    save)
      ...
      JSON="{\"taskId\":\"$TASKFLOW_TASK_ID\",\"flowId\":\"$TASKFLOW_FLOW_ID\",\"stepEntryId\":\"$TASKFLOW_STEP_ENTRY_ID\",\"type\":\"$TYPE\""
      [ -n "$ARTIFACT_PATH" ] && JSON="$JSON,\"path\":\"$ARTIFACT_PATH\""
      [ -n "$ARTIFACT_TEXT" ] && JSON="$JSON,\"text\":\"$ARTIFACT_TEXT\""
      JSON="$JSON}"
...
Create FlowRunner after ptyManager and taskStore are created. The `spawnSession` callback replicates the session creation logic from `handlers/session.ts` (lines 88-179):
...
const flowRunner = new FlowRunner({
  ...
  spawnSession: async (opts) => {
    ...
    if (opts.sessionType === "shell") {
      // Shell steps use the prompt as the command
      command = opts.prompt;
    } else {
...
Note: Because flow-spawned sessions call `flowRunner.handleSessionExit` directly in the `onExit` callback, the session exit handler in `handlers/session.ts` does NOT need modification for flow support. The FlowRunner's `spawnSession` callback handles both the session registration and the flow-specific exit logic.
...
interface FlowStore {
  ...
  // Active flow runs (keyed by taskId)
...
const useFlowStore = create<FlowStore>((set) => ({
  ...
  async fetchFlowRuns(taskId) {
    ...
    if (activeRun) {
      set((s) => ({ activeRuns: { ...s.activeRuns, [taskId]: activeRun } }));
    }
  },

  applyRunUpdate(run) {
    set((s) => ({ activeRuns: { ...s.activeRuns, [run.taskId]: run } }));
  },
}));
...
function StepEditor({ step, onSave, onCancel }: StepEditorProps) {
  ...
  const handleSave = useCallback(() => {
    ...
    onSave({
      ...
      prompt,
      sessionType,
      agentOptions: step?.agentOptions,
      createdAt: step?.createdAt ?? now,
      updatedAt: now,
    });
  }, [step, name, prompt, sessionType, onSave]);
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {/* Agent options (model select) can be added here following AgentOptionsPanel pattern */}
    ...
    <div className="flex flex-col gap-4 p-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex gap-1">
            {globalSteps.length > 0 && (
              /* Dropdown to pick from global steps — use DropdownMenu */
              <Button variant="outline" size="sm" onClick={() => {/* open step picker */}}>
        ...
        <div className="flex flex-col gap-1">
          {steps.map((entry, i) => (
            <div key={entry.id} className="flex items-center gap-2 p-2 bg-muted rounded border">
      ...
      <DialogContent className="max-w-3xl h-[70vh] flex p-0">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {tab === "steps" && (creating || selectedStep) && (
            <StepEditor step={creating ? null : selectedStep} onSave={handleSaveStep}
              onCancel={() => { setCreating(false); setSelectedId(null); }} />
          )}
...
import type { FlowStepState, FlowRun } from "@taskflow/shared";

interface FlowPanelProps {
  taskId: string;
  flowDefinitions: Array<{ id: string; name: string; steps: Array<{ id: string; label?: string; inline?: { name: string; sessionType: string }; stepId?: string }> }>;
}

function FlowPanel({ taskId, flowDefinitions }: FlowPanelProps) {
  const run = useFlowStore((s) => s.activeRuns[taskId]);
  const { fetchFlowRuns, skipStep, pauseFlow, resumeFlow, stopFlow, jumpToStep } = useFlowStore.getState();

  useEffect(() => { fetchFlowRuns(taskId); }, [taskId]);
  ...
  const getStepName = (state: FlowStepState, index: number): string => {
    ...
    return entry.label ?? entry.inline?.name ?? `Step ${index + 1}`;
  };

  const getStepSessionType = (index: number): string => {
    const entry = flowDef?.steps[index];
    return entry?.inline?.sessionType ?? "agent";
  };

  const handleJump = useCallback((index: number) => {
    if (confirm(`Re-run step "${getStepName(run.steps[index], index)}"?`)) {
      jumpToStep(taskId, run.flowId, index);
    }
  }, [run, taskId]);
...
Use `useFlowStore((s) => activeTaskId ? s.activeRuns[activeTaskId] : undefined)` to check for an active run. Only render the panel column when the prop is defined. Give it a fixed width (e.g., 200px) consistent with other side panels.
...
- [ ] **Step 3: Add flow selection to NewTaskDialog**

In `NewTaskDialog.tsx`, modify the "Start immediately" section. Currently it offers agent type selection. Add a new option to start with a flow instead:
...
//   - Hide agent options
// On submit: if flow selected, create task then start flow on it
```
...
```bash
git add packages/ui/src/components/AppShell.tsx packages/ui/src/components/workspace/TaskHeader.tsx packages/ui/src/components/sidebar/NewTaskDialog.tsx
...
Stage only the specific files that were changed to fix issues, then commit:

```bash
git commit -am "fix: address type errors and test regressions"
```
